...
3. Build `scripts/classify.py` (card rules plus Oscar's tags) and the queue store. `scripts/gmail_match.py` is done; Oscar ruled 2026-08-24: the Viseo AB forwarding identity is `oscar.sandstrom@viseo.se` (as the script already had it), Firecrawl belongs to Viseo AB and Oderland to 5555 Media AB (alias table updated). Still open: whether MCP attach replaces forwarding for all Viseo rows regardless of age.
