...
td .m{display:block;color:var(--muted);font-size:12px}td .looked{margin-top:3px;max-width:420px}td .step{display:block;color:var(--ink2);font-size:13px;margin-top:3px;max-width:420px}
...
${rows.length?rows.map(r=>`<tr><td>${esc(r.date)}</td><td>${esc(r.receipt_no)}${r.source==='live'?'<span class="m">efter exporten</span>':''}</td><td>${esc(r.vendor)}<span class="m">${esc(r.merchant)}${r.type&&r.type!=='Card Purchase'?' · '+esc(r.type):''}</span></td><td class="num">${sek(r.amount_sek)}</td><td class="c">${yn(r.has_receipt||r.state==='attached',r.payout)}</td><td>${esc(r.export_status_label)}</td><td>${badge('pleo',r.state)}<span class="step">${esc(r.step)}</span>${r.searched?'<span class="m looked">Var vi letat: '+esc(r.searched)+'</span>':''}</td></tr>`).join(''):'<tr><td colspan="7" class="empty">Inga rader matchar.</td></tr>'}</tbody></table></div>`}
