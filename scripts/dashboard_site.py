...
nav{display:flex;gap:6px;margin:12px 0 18px;border-bottom:1px solid var(--line);overflow-x:auto}nav button{background:none;border:0;border-bottom:3px solid transparent;padding:10px 14px;font:inherit;font-weight:600;color:var(--ink2);cursor:pointer}
...
.tile{display:flex;flex-direction:column}.tile .n{font-size:26px;font-weight:700;line-height:1.1;font-variant-numeric:tabular-nums;white-space:nowrap}.tile .l{color:var(--ink2);font-size:13px;margin-top:2px}.tile .s{color:var(--muted);font-size:12px;margin-top:4px}
...
.month{background:var(--card);border:1px solid var(--line);border-radius:10px;padding:10px 12px;display:flex;flex-direction:column}.month .h b{display:block;font-size:15px}.month .h .badge{margin-top:6px}
.month .r{font-size:13px;color:var(--ink2);margin-top:6px}.bar{display:flex;height:8px;border-radius:4px;overflow:hidden;background:var(--mutedbg);margin-top:8px;gap:2px}.month .grow{flex:1;min-height:2px}.bar i{display:block;height:100%}
...
@media (max-width:640px){.wrap{padding:12px 10px 40px}td .step,td .looked{max-width:none}td.st{min-width:220px}header h1{font-size:20px}.tiles{grid-template-columns:repeat(2,1fr)}.tools input{max-width:none}.tools .cnt{margin-left:0;width:100%}}
"""
...
return `<div class="month"><div class="h"><b>${esc(svMonth(m.month))}</b>${badge('stage',m.stage)}</div><div class="r">${m.rows} köp · ${m.business} företag · ${m.personal} privat${m.needs_tag_open?` · <b>${m.needs_tag_open} otaggade</b>`:''}</div><div class="r">Företag ${sek(m.business_sek)}</div><div class="r">Kvitto: ${f} av ${tot}</div><div class="grow"></div><div class="bar"><i class="g" style="flex:${f}"></i><i class="b" style="flex:${tot-f}"></i></div></div>`}
const ok=m.with_receipt,miss=m.missing;return `<div class="month"><div class="h"><b>${esc(svMonth(m.month))}</b><span class="badge ${miss?'bad':'good'}">${miss?miss+' utan kvitto':'kvitton klara'}</span></div><div class="r">${m.rows} rader · ${m.exported} exporterade${m.payouts?` · ${m.payouts} utbetalningar`:''}</div><div class="r">Summa ${sek(m.amount_sek)}</div><div class="r">Kvitto: ${ok} av ${ok+miss}</div><div class="grow"></div><div class="bar"><i class="g" style="flex:${ok}"></i><i class="b" style="flex:${miss}"></i></div></div>`}).join('')}</div>`}
